...
- Documentos de referencia obligada antes de tareas grandes: `docs/CONTEXTO_CLAUDE_CODE.md` (contexto y estado del prototipo), `docs/planificacion/plan_mejora_sae.md` (matriz de incisos y estados) y, para todo lo que toque el flujo de postulación o el simulador, `docs/planificacion/bitacora_flujo_postulacion_y_resultado.md` (decisiones + fundamento; su **sec. 7** tiene el roadmap vigente F1–F3 de la reorientación de la validación a estudio comparativo, y define el próximo trabajo de código: **F2** = endurecer la explicabilidad del prototipo, **F3** = modo control que oculta esa capa). El `CLAUDE.md` de la raíz tiene el estado del proyecto y las reglas globales; `archivo/CLAUDE_v2.md` es el que describe el prototipo HTML antiguo (solo referencia histórica). Para principios de diseño usa `NotasPage.jsx` y el `CLAUDE.md` raíz (divulgación progresiva, explicabilidad contextualizada, controles interactivos, paleta Mineduc), no para especificaciones técnicas.

## Validación (comandos reales)
...
3. `npm test` — runner nativo de Node (`node --test --import ./tests/setup.mjs "tests/**/*.test.js"`, **sin frameworks ni dependencias**; el hook `tests/setup.mjs` completa la extensión de los imports estilo Vite). `sae-react/tests/flujo-postulacion.test.js` (14 tests) ejercita `simulacionSae.js` + `asignacion.js` + `colegios.js` con el caso Muñoz González: snapshot calibrado de los 6 `%`, strategy-proofness, determinismo, el escenario clave y un guardarraíl sobre `PARAMS_POBLACION` / `SEED_CASO` / iteraciones. Córrelo en cada cambio que toque la lógica de asignación o los datos de `colegios.js`.
4. `npm run dev` — servidor de desarrollo para verificación manual cuando el usuario lo pida.

No agregues Vitest/Jest ni otros frameworks de test por iniciativa propia: el runner nativo de Node es una decisión deliberada del proyecto. Si una lógica necesita verificación puntual fuera de la suite, puedes ejecutarla con `node` de forma ad hoc y descartar el script después.
...
- `src/utils/simulacionSae.js` + `src/utils/asignacion.js`: lógica del simulador. **Reescrito el 2026-09-03 (bitácora Bloque R, `CONTEXTO_CLAUDE_CODE.md` §27, autorizado explícitamente por el usuario): ya NO existe la tabla `probAsignacion` ni el umbral de asignación 65.** El motor es `simulacionSae.js`: `probabilidadCupo()` estima el `%` por colegio con **Monte Carlo (`ITERACIONES_MONTE_CARLO` = 1000)** sobre una simulación DA-por-colegio — multitud sintética de `postulantesAnterior` competidores (dato real) con tramos de prioridad muestreados de `PARAMS_POBLACION`, cuota SEP del 15 % como sub-escuela (fase 1) + pozo general (fase 2), sorteo por colegio, todo con PRNG con semilla (`mulberry32`). `asignacion.js`: `calcularResultado()` saca el colegio asignado de un **recorrido determinista de la lista** con semilla fija `SEED_CASO = 20260903`; `probPorcentaje()` acota a `[1, 99]`; se conservan `nivelPrioridad()`, `nivelPrioridadEnColegio()`, `prioridadLabels` y `etiquetaPrioridad(nivel)` solo para **etiquetas** (ya no para el `%`). `etiquetaPrioridad` (2026-09-07) es el que se muestra al usuario: para `nivel 5` (sin vínculo) dice *"Sin vínculo con este colegio"* en vez de `prioridadLabels[5]` = *"Sorteo público transparente"* — "sorteo/azar" se reserva para explicar el mecanismo en `/algoritmo`, no para el encuadre en el flujo (criterio del usuario, ver `CONTEXTO_CLAUDE_CODE.md` §28). Las probabilidades son pedagógicas, no oficiales. **Núcleo protegido:** no cambies `PARAMS_POBLACION`, `SEED_CASO`, `ITERACIONES_MONTE_CARLO` ni el recorte `[1, 99]` sin petición explícita — hay un guardarraíl de regresión en `flujo-postulacion.test.js`. Limitaciones declaradas de la v1 (header de `simulacionSae.js`): DA por colegio y no multi-colegio; sin cuota PIE ni alta exigencia; `pHermano`/`pFuncionario`/`pExalumno` son estimaciones (solo `pSep` anclado a datos 2018).
- `src/data/colegios.js`: catálogo de 6 colegios ficticios (SIMCE, vacantes, `postulantesAnterior`, % docentes titulados, NEE, seguridad, demanda, `casoPrioridades` para el caso Muñoz González). Esquema v2 documentado; reutilizado por buscador, simulador, ficha, comparador y postulación. **El caso Muñoz González ya no usa cuota SEP ni PIE (bitácora Bloque O);** Colegio San Martín es `demanda: 'alta'` (el "colegio en mente" incierto, ~26 %). `postulantesAnterior` alimenta directamente la simulación: cualquier ajuste ahí mueve los `%` y debe pasar por `npm test`.
- `src/data/incisos.js` + `docs/planificacion/plan_mejora_sae.md`: matriz de cumplimiento de los incisos a–t del informe de calidad web. De aquí sale la cifra de trazabilidad que la memoria cita. Estado: **100 % (87/87 aplicables, 22 categorías)** desde el cierre de S22 el 2026-08-04. Las cifras 62/62 y 72/72 son históricas intermedias. El trabajo posterior a S22 se registra como `S<...>-<...> (refinamiento)` **sin sumar puntos**: el 87/87 no se toca.
- El trabajo sobre el flujo de postulación (`PostulacionPage.jsx`, `/perfil`, prioridad por colegio, gates de fidelidad) se documenta además en `docs/planificacion/bitacora_flujo_postulacion_y_resultado.md`, que mantiene `bitacora-agent` — tú no la editas; al cerrar una tarea de flujo, tu resumen le sirve de insumo.
...
1. `docs/planificacion/plan_mejora_sae.md` — si un inciso cambió de estado, actualiza su fila y el resumen final (conteo X/87 y porcentaje). El estado vigente es **87/87 (100 %), 22 categorías**, cerrado con S22 el 2026-08-04; el trabajo posterior se marca `S<...> (refinamiento)` sin sumar puntos. Si un cambio alterara esa cifra, dilo explícitamente en tu resumen para que el writing-agent sincronice.
