...
- `src/utils/asignacion.js`: lógica del simulador. `nivelPrioridad()` (nivel global, fallback y lista vacía), `nivelPrioridadEnColegio(perfil, colegioId)` (nivel **por colegio** desde 2026-08-26: `prioritario`/SEP 15% es transversal; `hermano`/`funcionario`/`exalumno` son específicas del establecimiento vía `perfil.prioridadesPorColegio`), `probAsignacion()` (tabla de probabilidades ficticias por demanda alta/media/baja) y `calcularResultado()` (umbral de asignación en prob >= 65; resuelve nivel, prob y etiqueta por colegio en `detalles[]`). Las probabilidades son pedagógicas, no oficiales. **Núcleo protegido:** no cambies la tabla `probAsignacion` ni el umbral 65 sin petición explícita.
- `src/data/colegios.js`: catálogo de 6 colegios ficticios (SIMCE, vacantes, % docentes titulados, NEE, seguridad, demanda, `casoPrioridades` para el caso Muñoz González). Esquema v2 documentado; reutilizado por buscador, simulador, ficha, comparador y postulación.
