...
**Refinamiento 2026-09-06:** `ResultadoProvisional` reescrito — se quitó la lógica de umbral 65 % (obsoleta desde Bloque R; ahora usa `resultado.sinAsignacionEnPreferencias` + `asignado.idx`), se suavizó la certeza (*"es poco probable"* / *"X de cada 100"* en vez de *"no quedarías… no alcanza"*), y **se eliminó todo lenguaje de "sorteo" / "azar"** (decisión del usuario: el proyecto combate el estigma de "tómbola"). El desenlace se atribuye a prioridades + vacantes + demanda; el resultado real se encuadra como *"lo calcula el sistema en octubre con las mismas reglas y las postulaciones reales de este año"* + enlace a `/algoritmo`. **Pendiente:** *"sorteo"*/*"azar"* siguen en `PrioridadColegioControl`, `prioridadLabels[5]` (`asignacion.js`) y el InfoBox inicial del paso 1 — reformularlas es una pasada de microcopy más amplia (decisión del usuario, toca S22-5).
