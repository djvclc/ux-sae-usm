...
**Segunda parte del mismo feedback — hermano/a ya matriculado/a (2026-09-13):** la prioridad "hermano/a matriculado/a" en Colegio Los Andes (`colegios.js casoPrioridades: ['hermano']`) era abstracta: el panel "esto es lo que el sistema ya sabe" solo decía el nombre del colegio, nunca de quién. `CASO_EJEMPLO` gana una identidad para ese hermano/a (**Antonia Muñoz González**, 6.º básico, Colegio Los Andes) y el "bloque de hermanos" del paso 1 muestra una tarjeta nueva **"Hermano/a ya matriculado/a"** con el mismo `VerificacionDatosCard` — Nombre / RUN / Nivel actual / **Colegio actual** —, ahora con `onCorregir` **opcional** (se omite: es un registro del Estado, no algo que se declare o corrija). Gate: solo con `perfilEstudiante.caso === 'munoz-gonzalez'` (mismo que `CondicionesDetectadas`/`mostrarVinculos`); con la identidad demo genérica no aparece. `npm run lint`/`build`/`test` (16/16) — verificado en navegador en ambos casos, sin desborde a 375 px. No entra en `calcularResultado`.
