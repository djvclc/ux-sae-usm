...
## 52. Grupo familiar como primera pantalla del paso 1 + seminegrita y contraste en todo el sitio + título "Postulación" (2026-09-22, feedback profesora guía)

- **Tú vs. usted:** se revisó cómo tratan al usuario los sitios del Estado — el sitio oficial del SAE ("Ingresa a la plataforma y revisa tu resultado") y ChileAtiende ("Revisa cómo cobrar tu PGU") tutean. Se **mantiene el tuteo**; sin cambios.
- **Contraste + negrita (todo el sitio):** `index.css` — `--gris` #333→#222, `--gris-med` #666→#404040 (10,4:1); `body` a `font-weight: 600`, `strong/b/h1–h6` a 800; los 12 `font-weight` 400/500/normal del CSS y los `fontWeight: 400` inline pasan a 600; `CardTitle` (`card.jsx`) de `font-medium` a `font-extrabold`. Se quitaron las `opacity` que atenuaban texto (`.criterio-card--sorteo`, `.ind-card--off`, `.post-colegio-fila--agregada`; `.chat-panel__sub` 0.75→0.95).
- **Título:** `<h1>` "Postular en 3 pasos" → "Postulación", sin la bajada técnica; la barra de progreso queda igual.
- **Grupo familiar (`PostulacionPage.jsx`, paso 1):** tras ClaveÚnica, pantalla "Tu grupo familiar" con los hijos/as (mayor → menor) como casillas, ninguno premarcado. `CASO_EJEMPLO.familia` = Martina (3.º medio, Los Andes), Mateo (6.º básico, Esc. Bernardo O'Higgins), Sofía (3.º básico, Esc. Básica Los Quillayes, caso_estudio §2); `IDENTIDAD_CLAVEUNICA_DEMO.familia` análogo (Ríos Contreras). Texto dinámico: 1 marcado → "Vas a postular solo a X" + "Y no postula: sigue en {colegio} y no pierde su matrícula"; 2+ → "Postulación en bloque" (intentan quedar en el mismo colegio; se revisa primero a quien es mayor…) + quién se queda. La demo arma **una** lista: la del/de la **menor** de los/as marcados/as (`postulantePrincipal`); los demás marcados son `hermanosEnBloque`. Luego un resumen con badges y "Cambiar". Se eliminaron los estados `hermanoNombre/Rut/Nivel/ColegioActual/hermanoMatriculado` y la sección "Hermanos y hermanas"; el payload guarda `hermanos: []` (antes `hermano`), el comprobante y el modal listan a todos los hermanos/as en bloque. `agregar()` solo siembra la prioridad "hermano" de `casoPrioridades` si **otro/a** integrante estudia hoy en ese colegio (evita que Martina tenga prioridad de hermana consigo misma). Textos del paso 1 reescritos (más cortos, "Revisa los datos de {nombre}").
- `asignacion.js`/`simulacionSae.js`/`colegios.js` intactos; caso canónico verificado (San Martín 26 % → Los Andes 99 %). `npm run lint` (0/0), `npm run build`, `npm test` (16/16). Verificado en navegador a 375 px (texto/DOM; la captura de pantalla sale en blanco en esta sesión). **Pendiente docs:** `caso_estudio…md` §6 / `guion_*` — la tarea 1 suma "elegir a quién postular" (en la prueba: Sofía y Mateo en bloque).

## 53. % estimado como chip en la fila de "Tu lista" + tarjeta de resultado de `/seguimiento` sin probabilidad ni demanda (2026-09-23, feedback profesora guía)

- **`PostulacionPage.jsx`, paso 2 "Tu lista":** se eliminó `ColegioAnalisis` (bloque gris `<details>` "Opción N · X% estimado" + barra + demanda/vacantes/prioridad bajo cada colegio: mucho alto para poco contenido). Nuevo `ProbabilidadChip` (chip `.tut-prob` + `.post-item__prob`) entre el nombre del colegio y los botones ↑/↓/Quitar; `aria-label` con "X de cada 100". Mismo gating que antes (`modoTutorial`, oculto con `esControl`). Lo demás ya estaba en la fila (n.º de opción, demanda/vacantes en la meta, prioridad en `PrioridadColegioControl`). Se quitó el import de `ProbabilidadVisual` de este archivo (sigue en `ColegioPage`) y el CSS `.tut-colegio-info*` sin uso. Verificado: 1100 px → nombre · chip · botones en una fila; 375 px → nombre + chip en una fila, botones debajo; sin overflow.
- **`SeguimientoPage.jsx`, `.seg-result-plano`:** se quitan las filas Probabilidad y Demanda (irrelevantes con el resultado ya disponible); "Prioridad:" muestra la etiqueta completa y la meta inferior deja solo la fecha (la prioridad salía dos veces). `.seg-result-plano__datos li` pasa de `justify-content: space-between` a etiqueta y valor juntos (gap 6 px).
- `asignacion.js`/`simulacionSae.js`/`colegios.js` intactos; canónico 26 % → 99 % (Los Andes, preferencia N.º 2). `npm run lint` (0/0), `npm run build`, `npm test` (16/16).
