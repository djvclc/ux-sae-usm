...
Los pendientes **P3–P7** no provienen de las guías de UX-IA sino de la tabla de brechas de fidelidad (sec. 2) de `analisis_video_paso_a_paso_sae.md` (2026-08-27): son brechas del flujo oficial que **no** se implementaron en el Bloque L. La brecha **H** (vista mapa de establecimientos) queda **fuera de alcance** (mobile-first, catálogo de 6 colegios) y no se registra como pendiente.
...
| **P3** | **Brecha B — dirección de residencia del estudiante:** el flujo real pide región + comuna + calle; el prototipo solo usa la región como filtro de exploración (S22-1), no captura comuna ni dirección. Implica capturar el dato y decidir si además se usa (proximidad en la asignación) | Fidelidad — `analisis_video_paso_a_paso_sae.md` sec. 2 (brecha B, **Indispensable**) · Paso 2 del video | Alta — pero **requiere decisión explícita del usuario** antes de implementarse (ver sec. 6 nº 8: alcance capturar vs. usar) | No |
| **P4** | **Brecha E — reenvío y comprobante actualizado al modificar:** en el flujo real, cada modificación durante el período obliga a reenviar la postulación y descargar un comprobante nuevo, y el anterior queda obsoleto. El prototipo dice "puedes modificar hasta el cierre" pero no que haya que reenviar ni que el comprobante previo se invalide | Fidelidad — `analisis_video_paso_a_paso_sae.md` sec. 2 (brecha E, **Deseable-alto**) · Paso 3 del video | Media-alta | No |
| **P5** | **Brecha F — obligación de postular sin continuidad de nivel:** el flujo real la enuncia como obligación explícita. El Bloque L (D) la cubre **parcialmente** (párrafo 3 de la InfoBox de alerta del paso 2); falta evaluar si merece tratamiento propio (p. ej. detección por el nivel del estudiante) | Fidelidad — `analisis_video_paso_a_paso_sae.md` sec. 2 (brecha F, **Deseable**, parcialmente cubierta por D) | Baja | No |
| **P6** | **Brecha G — IPA (identificador provisorio) como alternativa al RUT** en la identificación, para postulantes sin RUT | Fidelidad — `analisis_video_paso_a_paso_sae.md` sec. 2 (brecha G, **Deseable menor** para un prototipo pedagógico) | Baja | No |
| **P7** | **Brecha I — filtros de búsqueda de establecimientos:** jornada, mixto / un solo género, internado, técnico-profesional, dependencia (público / particular subvencionado), continuidad hasta enseñanza media. Varios ya existen como campos del colegio (`jornada`, `dependencia`, `orientacion`) pero no como filtros en `InicioPage` | Fidelidad — `analisis_video_paso_a_paso_sae.md` sec. 2 (brecha I, **Deseable**, encontrabilidad S5) | Baja | No |
...
8. **Brecha B — dirección de residencia del estudiante (pendiente P3 de la sec. 5).** Capturar región + comuna + calle como en el flujo oficial **requiere decisión explícita del usuario** antes de implementarse: hay que definir si el dato solo se captura (fidelidad del formulario) o si además se usa para algo (p. ej. proximidad en la asignación). No entra en ningún ciclo mientras no se resuelva.
