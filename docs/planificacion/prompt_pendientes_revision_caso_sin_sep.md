...
**Para qué es esto:** el 2026-09-03 se revisó el caso de estudio de la prueba de usabilidad (Bloque O) y, la misma fecha, se **reemplazó el motor de probabilidad del prototipo** por una simulación DA-por-colegio con Monte Carlo (Bloque R). La documentación de `docs/` ya está al día; falta lo de `proyecto-tesis/` (writing-agent) y una regeneración de `material_prueba_usabilidad_postulacion.pdf`. Este archivo es un prompt listo para pegar en una sesión nueva.
